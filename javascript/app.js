...
function getUser() {
  try { return JSON.parse(localStorage.getItem('kitabi_user')); } 
  catch { return null; }
}
...
function savePanier(panierElements) { localStorage.setItem('kitabi_panier', JSON.stringify(panierElements)); }
function ajouterPanier(livre) {
  const panier = getPanier();
  const existe = panier.find(item => item.id === livre.id);
  if (existe) { existe.qte++; } else { panier.push({ ...livre, qte: 1 }); }
  savePanier(panier);
  updateCompteurs();
}
...
function changerQte(id, qte) {
  ...
  if (item) { item.qte = Math.max(1, qte); savePanier(panier); }
  updateCompteurs();
}
function totalPanier() { return getPanier().reduce((somme, item) => somme + (item.prix * item.qte), 0); }
...
function isFavori(id) { return getFavoris().some(item => item.id === id); }
function toggleFavori(livre) {
  let favoris = getFavoris();
  if (isFavori(livre.id)) { favoris = favoris.filter(item => item.id !== livre.id); } 
  else { favoris.unshift(livre); }
...
function updateCompteurs() {
  ...
  if (badgePanier) { badgePanier.textContent = totalArticles; badgePanier.classList.toggle('hidden', totalArticles === 0); }
  if (badgeFavoris) { badgeFavoris.textContent = totalFavoris; badgeFavoris.classList.toggle('hidden', totalFavoris === 0); }
}
...
function buildNavbar() {
  ...
  conteneurNav.innerHTML = `
    <div class="nav-inner">
      ...
      <div class="nav-icons">
        <a href="${prefixe}favoris.html" class="icon-btn" title="Favoris">♥ <span id="badge-favoris" class="badge hidden">0</span></a>
        <a href="${prefixe}panier.html" class="icon-btn" title="Panier">🛒 <span id="badge-panier" class="badge hidden">0</span></a>
        ${user ? `<a href="${prefixe}compte.html" class="btn-outline-sm">👤 ${user.nom}</a><button onclick="doLogout()" class="btn-outline-sm" title="Déconnexion">↩</button>` : `<a href="${prefixe}connexion.html" class="btn-outline-sm">Se connecter</a>`}
      </div>
    </div>`;
  updateCompteurs();
}

// 6. MODULES CARTES ET COUVERTURES
function coverUrl(livre) {
  if (livre.image) {
    const estSurAccueil = window.location.pathname.endsWith("index.html") || window.location.pathname.endsWith("/");
    let cheminNettoye = livre.image;
    if (cheminNettoye.startsWith("../")) { cheminNettoye = cheminNettoye.replace("../", ""); }
    return estSurAccueil ? cheminNettoye : "../" + cheminNettoye;
  }
...
{ const imageSrc = coverUrl(livre); 
 ...
 return ` <div class="card" data-id="${livre.id}"> <!-- Seul le cadre de couverture coloré ouvre les détails du livre au clic --> <div class="card-cover" style="background:${couleurFond}; cursor: pointer;" onclick="window.location.href='detail.html?id=${livre.id}'"> ${imageSrc ? `<img src="${imageSrc}" alt="${livre.titre}" onerror="this.style.display='none'">` : ''} <div class="card-cover-text"> <span>${livre.titre}</span> <small>${livre.auteur}</small> </div> </div> <button class="heart-btn ${favoriActif ? 'active' : ''}" onclick="event.preventDefault(); toggleFavori(livres[${index}]); this.classList.toggle('active')" title="Favoris">♥</button> <div class="card-body"> <span class="badge-cat">${livre.categorie}</span> <h3>${livre.titre}</h3> <p class="auteur">${livre.auteur}</p> <div class="card-footer"> <strong class="prix">${livre.prix.toLocaleString('fr-DZ')} DA</strong> <button class="btn btn-sm" onclick="ajouterPanier(livres[${index}]); this.textContent='✓'; setTimeout(() => this.textContent='+ Panier', 1500)"> + Panier </button> </div> </div> </div>`; }
